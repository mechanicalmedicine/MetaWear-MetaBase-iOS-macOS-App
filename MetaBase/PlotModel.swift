...
protocol PlotModelDelegate: class {
...
class PlotModel {
    public weak var delegate: PlotModelDelegate?
    var device: MetaWear?
    ...
    public func startWalkthrough() {
        welcomeScreen().continueOnSuccessWithTask {
            ...
            self.delegate?.plottingComplete(device: self.device, error: $0.error)
        }
    }
    ...
    public func skip() {
        self.device?.cancelConnection()
        self.delegate?.plottingComplete(device: self.device, error: MetaWearError.operationFailed(message: "Plotting skipped"))
    }
    ...
    func welcomeScreen() -> Task<Void> {
        ...
        delegate?.updateUI(message: "Plot the data.", icon: nil, buttonText: "NEXT", chart: false)
